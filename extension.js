...
const toStackTrace = (text, callback) => {
	try {
	  ...
	  json.frames.forEach(element => {
		...
		ret += "at " + element.class+"."+element.method + "(" + javafile + ":" + element.line + ")\n";
	  });
	  callback(null, ret);
	} catch (e) {
...
function activate(context) {
	context.subscriptions.push(
		vscode.commands.registerCommand("json-to-java-stack-trace.toStackTrace", convertSelection(toStackTrace))
	  );
}
